...
def ffwd(data_in, paths_out, model, device_t='', batch_size=1):
    ...
    if is_paths:
        img_shape = get_img(data_in[0], img_size=(256, 256, 3)).shape
    else:
    ...
    config.mlu_options.core_version = "MLU270"                if is_paths:
                    X = np.zeros(batch_shape, dtype=np.float32)
                    for j, path_in in enumerate(data_in[pos: pos + batch_size]):
                        img = get_img(path_in, img_size=(256, 256, 3))
                        X[j] = img
                else:
