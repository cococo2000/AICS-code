7.2 实验编译与运行
    1、实验可选择bangc或者bangpy实现
    bangc算子填写：补齐cnplugin-SBC/spilt_sub_concat_kernel.hh和spilt_sub_concat_kernel.mlu。执行make进行编译，生成可执行文件test，进行测试。
    bangpy算子编写：补齐bangpy/sbc.py，执行python3 sbc.py，生成spilt_sub_concat_kernel.mlu。执行make进行编译，生成可执行文件test，进行测试。
...
score: 90 * 0.8 = 72
best delay optimization: 39.8ms
