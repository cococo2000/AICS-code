...
//TODO:补齐下面create和compute
tensorflow::Status CreateBatchMatMulV2Op(MLUBaseOp** op,
  ...
  inputs[1] = input2;
  outputs[0] = output;
  CNML_RETURN_STATUS(cnmlCreatePluginBatchMatMulV2Op(op, param, inputs, outputs));
}

tensorflow::Status ComputeBatchMatMulV2Op(MLUBaseOp* op, MLUCnrtQueue* queue,
                                          void** inputs, int input_num,
                                          void** outputs, int output_num) {
  // ......
  CNML_RETURN_STATUS(cnmlComputePluginBatchMatMulV2OpForward(
      op, inputs, input_num, outputs, output_num, queue));
}
