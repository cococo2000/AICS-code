...
#if CAMBRICON_MLU
#include "tensorflow/core/kernels/batch_matmul_op_mlu.h"
#include "tensorflow/core/kernels/batch_matmul_v2_op_mlu.h"
#endif
...
class CublasScratchAllocator : public se::ScratchAllocator {
 public:
  ...
  CublasScratchAllocator(OpKernelContext* context) : context_(context) {}  // NOLINT
...
//TODO:添加MLUBatchMatMulV2算子注册
#if CAMBRICON_MLU
#define REGISTER_BATCH_MATMUL_MLU(T)                               \
REGISTER_KERNEL_BUILDER(                                           \
    Name("BatchMatMul").Device(DEVICE_MLU).TypeConstraint<T>("T"), \
    MLUBatchMatMul<T>);                                            \
REGISTER_KERNEL_BUILDER(                                            \
    Name("BatchMatMulV2").Device(DEVICE_MLU).TypeConstraint<T>("T"),\
    MLUBatchMatMulV2<T>)
#endif  // CAMBRICON_MLU
}  // namespace tensorflow
// REGISTER_KERNEL_BUILDER(......)
