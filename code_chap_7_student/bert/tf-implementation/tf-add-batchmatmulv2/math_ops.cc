...
REGISTER_OP("MLUBatchMatMulV2")
    ...
    .Output("output: T")
    .Attr("T: {half, float}")
    .Attr("input1_position: int = 0")
    .Attr("input1_scale: float = 1.0")
    .Attr("input2_position: int = 0")
    .Attr("input2_scale: float = 1.0")
    .Attr("adj_x: bool = false")
    .Attr("adj_y: bool = false")
    .SetShapeFn(shape_inference::BatchMatMulV2Shape)
    .Doc(R"doc(
MLUBatchMatMulV2 (MLU only)
)doc");
...
//    .SetShapeFn(shape_inference::UnchangedShape)
//    .Doc(R"doc(
// Convert input of type float32 to output of type int8.
// )doc");
...
REGISTER_OP("MLULess").MLUCOMPARISON();

REGISTER_OP("MLULessEqual").MLUCOMPARISON();

REGISTER_OP("MLUGreater").MLUCOMPARISON();

REGISTER_OP("MLUGreaterEqual").MLUCOMPARISON();
...
#if CAMBRICON_MLU
#define MLU_EQUALITY_COMPARISON()                                          \
  Input("x: T")                                                            \
      .Input("y: T")                                                       \
      .Output("z: T")                                                      \
      .SetIsCommutative()                                                  \
...
REGISTER_OP("MLUEqual").MLU_EQUALITY_COMPARISON();

REGISTER_OP("MLUNotEqual").MLU_EQUALITY_COMPARISON();

#undef MLU_EQUALITY_COMPARISON
#endif  // CAMBRICON_MLU
...
#if CAMBRICON_MLU
REGISTER_OP("MLULogicalNot")
    .Input("x: T")
    .Output("y: T")
    .Attr("T: {half, float}")
    .SetShapeFn(shape_inference::UnchangedShape);

#define MLU_BINARY_LOGICAL()      \
  Input("x: T")                   \
      .Input("y: T")              \
      .Output("z: T")             \
      .Attr("T: {half, float}")   \
...
REGISTER_OP("MLULogicalAnd").MLU_BINARY_LOGICAL();

REGISTER_OP("MLULogicalOr").MLU_BINARY_LOGICAL();

#undef MLU_BINARY_LOGICAL
#endif  // CAMBRICON_MLU
// --------------------------------------------------------------------------

Status SelectShapeFn(InferenceContext* c) {
...
REGISTER_OP("Select")
    ...
    .Output("output: T")
    .Attr("T: type")
    .SetShapeFn(SelectShapeFn);

#if CAMBRICON_MLU
REGISTER_OP("MLUSelect")
    .Input("condition: T")
    .Input("t: T")
    .Input("e: T")
    .Output("output: T")
...
REGISTER_OP("QuantMatMul")
    .Input("x: T")
    .Input("w: T")
    .Input("input_quantify_param: float")
    .Input("weight_quantify_param: float")
    ...
    .Doc(R"doc(
QuantMatMul (MLU only): similar to gen_math_ops.mat_mul(), return `x` (*) `w`.
The difference bewteen QuantMatMul and gen_math_ops.mat_mul() is the inputs type of QuantMatMul is intx, and the inputs type mat_mul is float.
)doc");

REGISTER_OP("QuantMatMulBias")
    .Input("x: T")
    .Input("w: T")
    .Input("input_quantify_param: float")
    .Input("weight_quantify_param: float")
    ...
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
QuantMatMul (MLU only): similar to gen_math_ops.mat_mul(), return `x` (*) `w` + `bias`.
The difference bewteen QuantMatMul and gen_math_ops.mat_mul() is the inputs type of QuantMatMul is intx, and the inputs type mat_mul is float.
)doc");
...
REGISTER_OP("MLP")
    ...
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
MLP (MLU only): similar to tf.nn.xw_plus_b, return `x` (*) `w` + `b`.
The difference bewteen MLP and xw_plus_b is that in MLP every batch of `x` is multiplied(*) with every row of `w`, instead of column.
)doc");
...
REGISTER_OP("Fix8MLP")
    ...
    .Attr("T1: {int8, float}")
    .Attr("axis: int = 1")
    .Attr("input_position: int = 0")
    .Attr("input_scale: float = 1.0")
    .Attr("filter_position: int = 0")
    .Attr("filter_scale: float = 1.0")
    .Attr("transpose_a: bool = false")
    .Attr("transpose_b: bool = false")
    .Attr("b_is_sparse: bool = false")
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
Fix8MLP (MLU only): similar to tf.nn.xw_plus_b, return `x` (*) `w` + `b`.
The difference bewteen MLP and xw_plus_b is that in MLP every batch of `x` is multiplied(*) with every row of `w`, instead of column.
)doc");

REGISTER_OP("Fix8MatMul")
    .Input("a: T")
    .Input("b: T1")
    .Output("product: T")
    .Attr("T: {half, float, double}")
    .Attr("T1: {int8, half, float, double}")
    .Attr("axis: int = 1")
    .Attr("input_position: int = 0")
    .Attr("input_scale: float = 1.0")
    .Attr("filter_position: int = 0")
    ...
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
Fix8MatMul (MLU only): similar to tf.nn.xw_plus_b, return `x` (*) `w` + `b`.
The difference bewteen MLP and xw_plus_b is that in MLP every batch of `x` is multiplied(*) with every row of `w`, instead of column.
)doc");
REGISTER_OP("Int8MLP")
    .Input("a: T")
    ...
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
Int8MLP (MLU only): similar to tf.nn.xw_plus_b, return `x` (*) `w` + `b`.
The difference bewteen MLP and xw_plus_b is that in MLP every batch of `x` is multiplied(*) with every row of `w`, instead of column.
)doc");
REGISTER_OP("Int16MLP")
    .Input("a: T")
    .Input("b: T1")
    .Input("bias: T")
    .Output("product: T")
...
REGISTER_OP("Int8MatMul")
    ...
    .SetShapeFn(shape_inference::MatMulShape)
    .Doc(R"doc(
Int8MatMul (MLU only): similar to tf.nn.xw_plus_b, return `x` (*) `w` + `b`.
The difference bewteen MLP and xw_plus_b is that in MLP every batch of `x` is multiplied(*) with every row of `w`, instead of column.
)doc");
REGISTER_OP("Int16MatMul")
    .Input("a: T")
    .Input("b: T1")
    .Output("product: T")
    .Attr("T: {half, float}")
...
REGISTER_OP("Quantify")
    .Input("input: float")
    .Input("oldquanparams: float")
    .Input("oldmovpos: float")
    .Input("interval: int32")
    .Output("output: T")
    .Output("quanparams: float")
    .Output("movpos: float")
    .Attr("T: {int8, int16}")
    .SetShapeFn([](InferenceContext* c) {
...
REGISTER_OP("FakeQuantInt8")
    .Input("inputs: float")
    .Input("in_point: float")
    .Output("output: float")
    .Output("out_point: float")
    .SetShapeFn(shape_inference::UnchangedShape)
    .Doc(R"doc(
Convert input data type from float to fixed float.
)doc");

REGISTER_OP("FakeQuantScaleInt8")
    .Input("inputs: float")
    .Output("output: float")
    ...
    .Doc(R"doc(
Convert input data type from float to fixed float.
This is new tranform function.
)doc");

REGISTER_OP("FakeQuantScaleInt8Gen")
    .Input("inputs: float")
    .Output("output: float")
    .Output("scale: float")
    .Output("pos: int32")
...
REGISTER_OP("FakeQuantScaleInt16Gen")
    .Input("inputs: float")
    .Output("output: float")
    .Output("scale: float")
    .Output("pos: int32")
