...
//TODO:补齐下面函数
  Status BatchMatMulV2(OpKernelContext* ctx,
    Tensor* input1, Tensor* input2,
    float scale_0, int pos_0, float scale_1,
    int pos_1, int dim_0, int dim_1, int m,
    int n, int k, Tensor* output) {
    // ......
    ops::MLUBatchMatMulV2OpParam op_param(scale_0, pos_0, scale_1,
        pos_1, dim_0, dim_1, m, n, k);
    return CommonOpImpl<ops::MLUBatchMatMulV2>(ctx, {input1, input2},
        {output}, static_cast<void*>(&op_param));
  }
